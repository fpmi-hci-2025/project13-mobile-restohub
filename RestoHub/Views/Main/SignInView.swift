import SwiftUI

struct SignInView: View {
    @Environment(\.presentationMode) var presentationMode
    @EnvironmentObject var userViewModel: UserViewModel
    
    @State private var login = "" // Общее поле для email/телефона
    @State private var password = ""
    @State private var rememberMe = false
    @State private var showingSignUp = false
    @State private var selectedLanguage = "English"
    @State private var showingCustomMenu = false
    @State private var loginMethod: LoginMethod = .email
    
    // Состояние для отслеживания фокуса
    @FocusState private var focusedField: Field?
    
    enum Field {
        case login, password
    }
    
    enum LoginMethod {
        case email
        case phone
    }
    
    var body: some View {
        NavigationView {
            ZStack {
                // ЧЕРНЫЙ ФОН
                Color.black
                    .ignoresSafeArea()
                
                VStack(spacing: 0) {
                    // Верхняя панель - ЧЕРНАЯ
                    HStack {
                        Button(action: {
                            presentationMode.wrappedValue.dismiss()
                        }) {
                            Image(systemName: "chevron.left")
                                .font(.system(size: 20, weight: .medium))
                                .foregroundColor(.white)
                        }
                        .padding(.leading, 16)
                        
                        Spacer()
                        
                        Text("Sign In")
                            .font(.system(size: 18, weight: .semibold))
                            .foregroundColor(.white)
                        
                        Spacer()
                        
                        // Кнопка смены языка - ОРАНЖЕВАЯ
                        Button(action: {
                            showingCustomMenu = true
                        }) {
                            HStack(spacing: 6) {
                                Text(selectedLanguage)
                                    .font(.system(size: 12, weight: .medium))
                                    .foregroundColor(.black)
                                
                                Image(systemName: "chevron.down")
                                    .font(.system(size: 10, weight: .medium))
                                    .foregroundColor(.black)
                            }
                            .padding(.horizontal, 10)
                            .padding(.vertical, 4)
                            .background(Color.orange)
                            .cornerRadius(6)
                        }
                        .padding(.trailing, 16)
                    }
                    .frame(height: 60)
                    .background(Color.black)
                    
                    ScrollView {
                        VStack(spacing: 30) {
                            // Логотип - ОРАНЖЕВЫЙ
                            VStack(spacing: 8) {
                                Text("RestoHUB")
                                    .font(.system(size: 36, weight: .bold))
                                    .foregroundColor(.orange)
                                
                                Text("Welcome to RestoHUB")
                                    .font(.system(size: 16))
                                    .foregroundColor(.orange.opacity(0.9))
                            }
                            .padding(.top, 40)
                            
                            // Переключатель способа входа
                            HStack(spacing: 0) {
                                Button(action: {
                                    withAnimation {
                                        loginMethod = .email
                                        login = "" // Очищаем поле при смене метода
                                    }
                                }) {
                                    VStack(spacing: 8) {
                                        Text("Email")
                                            .font(.system(size: 16, weight: .medium))
                                            .foregroundColor(loginMethod == .email ? .white : .white.opacity(0.6))
                                        
                                        Rectangle()
                                            .frame(height: 2)
                                            .foregroundColor(loginMethod == .email ? .orange : .clear)
                                    }
                                    .frame(maxWidth: .infinity)
                                }
                                
                                Button(action: {
                                    withAnimation {
                                        loginMethod = .phone
                                        login = "" // Очищаем поле при смене метода
                                    }
                                }) {
                                    VStack(spacing: 8) {
                                        Text("Phone")
                                            .font(.system(size: 16, weight: .medium))
                                            .foregroundColor(loginMethod == .phone ? .white : .white.opacity(0.6))
                                        
                                        Rectangle()
                                            .frame(height: 2)
                                            .foregroundColor(loginMethod == .phone ? .orange : .clear)
                                    }
                                    .frame(maxWidth: .infinity)
                                }
                            }
                            .padding(.horizontal, 24)
                            .padding(.bottom, 10)
                            
                            // Поля формы
                            VStack(spacing: 20) {
                                VStack(alignment: .leading, spacing: 8) {
                                    Text(loginMethod == .email ? "EMAIL / USERNAME" : "PHONE")
                                        .font(.system(size: 12, weight: .medium))
                                        .foregroundColor(.orange)
                                    
                                    ZStack(alignment: .leading) {
                                        if loginMethod == .email {
                                            TextField("", text: $login)
                                                .keyboardType(.emailAddress)
                                                .autocapitalization(.none)
                                                .padding()
                                                .background(Color.black)
                                                .cornerRadius(8)
                                                .foregroundColor(.white)
                                                .overlay(
                                                    RoundedRectangle(cornerRadius: 8)
                                                        .stroke(Color.orange, lineWidth: 1)
                                                )
                                                .focused($focusedField, equals: .login)
                                        } else {
                                            TextField("", text: $login)
                                                .keyboardType(.phonePad)
                                                .padding()
                                                .background(Color.black)
                                                .cornerRadius(8)
                                                .foregroundColor(.white)
                                                .overlay(
                                                    RoundedRectangle(cornerRadius: 8)
                                                        .stroke(Color.orange, lineWidth: 1)
                                                )
                                                .focused($focusedField, equals: .login)
                                        }
                                        
                                        // БЕЛЫЙ пример ввода (показывается только когда поле пустое и не в фокусе)
                                        if login.isEmpty && focusedField != .login {
                                            Text(loginMethod == .email ? "Please email@com/yournickname" : "+375 29 123-45-67")
                                                .foregroundColor(.white.opacity(0.7))
                                                .padding(.horizontal, 16)
                                                .allowsHitTesting(false)
                                        }
                                    }
                                }
                                
                                VStack(alignment: .leading, spacing: 8) {
                                    Text("PASSWORD")
                                        .font(.system(size: 12, weight: .medium))
                                        .foregroundColor(.orange)
                                    
                                    ZStack(alignment: .leading) {
                                        SecureField("", text: $password)
                                            .padding()
                                            .background(Color.black)
                                            .cornerRadius(8)
                                            .foregroundColor(.white)
                                            .overlay(
                                                RoundedRectangle(cornerRadius: 8)
                                                    .stroke(Color.orange, lineWidth: 1)
                                            )
                                            .focused($focusedField, equals: .password)
                                        
                                        // БЕЛЫЙ пример ввода для пароля (показывается только когда поле пустое и не в фокусе)
                                        if password.isEmpty && focusedField != .password {
                                            Text("••••••••")
                                                .foregroundColor(.white.opacity(0.7))
                                                .padding(.horizontal, 16)
                                                .allowsHitTesting(false)
                                        }
                                    }
                                }
                                
                                HStack {
                                    // Улучшенный переключатель Remember me
                                    HStack(spacing: 8) {
                                        Toggle("", isOn: $rememberMe)
                                            .toggleStyle(SwitchToggleStyle(tint: .orange))
                                            .labelsHidden()
                                            .scaleEffect(0.9)
                                        
                                        Text("Remember me")
                                            .font(.system(size: 14))
                                            .foregroundColor(.orange)
                                            .onTapGesture {
                                                rememberMe.toggle()
                                            }
                                    }
                                    .padding(8)
                                    
                                    Spacer()
                                    
                                    Button("Forgot password?") {
                                        // Действие для восстановления пароля
                                    }
                                    .font(.system(size: 14, weight: .medium))
                                    .foregroundColor(.orange)
                                }
                                .padding(.top, 10)
                            }
                            .padding(.horizontal, 24)
                            
                            // Кнопка входа - ОРАНЖЕВАЯ
                            Button(action: {
                                if loginMethod == .email {
                                    userViewModel.signIn(email: login, password: password)
                                } else {
                                    userViewModel.signIn(phone: login, password: password)
                                }
                                presentationMode.wrappedValue.dismiss()
                            }) {
                                Text("Sign in")
                                    .font(.system(size: 16, weight: .bold))
                                    .foregroundColor(.white)
                                    .frame(maxWidth: .infinity)
                                    .padding(.vertical, 18)
                                    .background(Color.orange)
                                    .cornerRadius(12)
                            }
                            .padding(.horizontal, 24)
                            .shadow(color: Color.orange.opacity(0.5), radius: 10, x: 0, y: 5)
                            
                            // Ссылка на регистрацию
                            HStack {
                                Text("Don't have an account yet?")
                                    .font(.system(size: 14))
                                    .foregroundColor(.orange)
                                
                                Button("Sign up") {
                                    showingSignUp = true
                                }
                                .font(.system(size: 14, weight: .semibold))
                                .foregroundColor(.orange)
                            }
                            .padding(.top, 20)
                            
                            Spacer()
                                .frame(height: 50)
                        }
                    }
                    .background(Color.black)
                    .onTapGesture {
                        // Скрываем клавиатуру при тапе вне полей ввода
                        focusedField = nil
                    }
                }
                .navigationBarHidden(true)
                .background(Color.black)
                
                // Кастомное меню выбора языка с ЧЕРНЫМ фоном
                if showingCustomMenu {
                    Color.black.opacity(0.7)
                        .edgesIgnoringSafeArea(.all)
                        .onTapGesture {
                            showingCustomMenu = false
                        }
                    
                    VStack(spacing: 0) {
                        VStack(spacing: 0) {
                            Button(action: {
                                selectedLanguage = "English"
                                showingCustomMenu = false
                            }) {
                                HStack {
                                    Text("English")
                                        .font(.system(size: 14))
                                        .foregroundColor(.orange)
                                    
                                    Spacer()
                                    
                                    if selectedLanguage == "English" {
                                        Image(systemName: "checkmark")
                                            .foregroundColor(.orange)
                                            .font(.system(size: 12))
                                    }
                                }
                                .padding(.horizontal, 16)
                                .padding(.vertical, 12)
                                .frame(maxWidth: .infinity, alignment: .leading)
                                .background(Color.black)
                            }
                            
                            Divider()
                                .background(Color.orange.opacity(0.3))
                                .padding(.horizontal, 16)
                            
                            Button(action: {
                                selectedLanguage = "Русский"
                                showingCustomMenu = false
                            }) {
                                HStack {
                                    Text("Русский")
                                        .font(.system(size: 14))
                                        .foregroundColor(.orange)
                                    
                                    Spacer()
                                    
                                    if selectedLanguage == "Русский" {
                                        Image(systemName: "checkmark")
                                            .foregroundColor(.orange)
                                            .font(.system(size: 12))
                                    }
                                }
                                .padding(.horizontal, 16)
                                .padding(.vertical, 12)
                                .frame(maxWidth: .infinity, alignment: .leading)
                                .background(Color.black)
                            }
                        }
                        .background(Color.black)
                    }
                    .background(Color.black)
                    .overlay(
                        RoundedRectangle(cornerRadius: 8)
                            .stroke(Color.orange, lineWidth: 1)
                    )
                    .frame(width: 180)
                    .shadow(color: Color.orange.opacity(0.3), radius: 15, x: 0, y: 8)
                    .position(x: UIScreen.main.bounds.width - 110, y: 100)
                    .transition(.scale.combined(with: .opacity))
                    .zIndex(1000)
                }
            }
        }
        .fullScreenCover(isPresented: $showingSignUp) {
            SignUpView()
        }
        .animation(.spring(response: 0.3), value: showingCustomMenu)
    }
}
